...
use staking::test_utils::constants::{
    AVG_BLOCK_DURATION, BTC_18D_CONFIG, BTC_5D_CONFIG, BTC_8D_CONFIG, EPOCH_DURATION, EPOCH_LENGTH,
    PUBLIC_KEY, STRK_BASE_VALUE, TEST_MIN_BTC_FOR_REWARDS,
};
use staking::test_utils::{
    StakingInitConfig, advance_blocks, calculate_staker_btc_pool_rewards_v2,
    calculate_staker_btc_pool_rewards_v3, calculate_staker_strk_rewards_with_balances_v2,
...
fn update_rewards_delegator_rewards_same_epoch_flow_test() {
    ...
    let stake_amount = system.staking.get_min_stake();
    let delegation_amount = STRK_CONFIG.min_for_rewards;
    let quarter_epoch_length = (EPOCH_LENGTH / 4).into();
    let staker = system.new_staker(amount: stake_amount);
    let commission = 200;
    system.stake(:staker, amount: stake_amount, pool_enabled: true, :commission);
    let pool = system.staking.get_pool(:staker);
    ...
    system.update_rewards(:staker, disable_rewards: false);
    advance_block_number_global(blocks: quarter_epoch_length);
    system.update_rewards(:staker, disable_rewards: false);
    advance_block_number_global(blocks: quarter_epoch_length);
    system.update_rewards(:staker, disable_rewards: false);
    advance_block_number_global(blocks: quarter_epoch_length);
