...
use test_utils::{
    ...
    declare_staking_eic_contract_v0_v1, deploy_mock_erc20_contract, deploy_reward_supplier_contract,
...
fn test_get_pool_exit_intent() {
    ...
    let expected = UndelegateIntentValue {
        unpool_time: Time::now().add(delta: exit_wait_window),
        amount: 2,
        staker_address: cfg.test_info.staker_address,
    };
...
fn test_undelegate_intent_is_non_zero() {
    let d = UndelegateIntentValue {
        unpool_time: UNPOOL_TIME, amount: 1, staker_address: Zero::zero(),
    };
    assert!(!d.is_zero());
    assert!(d.is_non_zero());
}
...
fn test_undelegate_intent_is_valid() {
    ...
    assert!(d.is_valid());
    let d = UndelegateIntentValue {
        unpool_time: Zero::zero(), amount: 1, staker_address: Zero::zero(),
    };
    assert!(!d.is_valid());
    let d = UndelegateIntentValue {
        unpool_time: UNPOOL_TIME, amount: Zero::zero(), staker_address: Zero::zero(),
    };
    assert!(!d.is_valid());
}
...
fn test_staking_eic() {
    ...
    let pool_contract_admin = *snforge_std::load(
        ...
        .at(0);
    assert_eq!(pool_contract_admin, MAINNET_SECURITY_COUNSEL_ADDRESS().into());
}

// TODO: Test when eic for the new version is implemented.
#[test]
...
fn test_staking_eic_with_wrong_number_of_data_elemnts() {
    ...
    // Upgrade.
    let eic_data = EICData {
        eic_hash: declare_staking_eic_contract_v0_v1(), eic_init_data: [].span(),
    };
    let implementation_data = ImplementationData {
        impl_hash: declare_staking_contract(), eic_data: Option::Some(eic_data), final: false,
    };
