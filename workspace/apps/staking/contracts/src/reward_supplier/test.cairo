...
use staking::flow_test::utils::{declare_reward_supplier_contract, upgrade_implementation};
...
use test_utils::{
    ...
    advance_k_epochs_global, advance_time_global, declare_reward_supplier_eic_contract, fund,
...
fn test_update_current_epoch_block_rewards_with_adjustments() {
    ...
    let mut curr_avg_block_time = DEFAULT_AVG_BLOCK_TIME;
    // Adjust avg_block_time to MIN (avg is less than min).
    let min_block_time = DEFAULT_BLOCK_TIME_CONFIG.min_block_time;
    advance_epoch_global_custom_time(
        block_time: TimeDelta { seconds: min_block_time / BLOCK_TIME_SCALE - 1 },
    );
    cheat_caller_address_once(contract_address: reward_supplier, caller_address: staking_contract);
    let (strk_rewards, btc_rewards) = reward_supplier_dispatcher
        .update_current_epoch_block_rewards();
    // Update the expected avg_block_time.
    let weighted_avg_factor = DEFAULT_BLOCK_TIME_CONFIG.weighted_avg_factor;
    curr_avg_block_time =
        (weighted_avg_factor.into() * min_block_time
    ...
    let max_block_time = DEFAULT_BLOCK_TIME_CONFIG.max_block_time;
    advance_epoch_global_custom_time(
        block_time: TimeDelta { seconds: max_block_time / BLOCK_TIME_SCALE + 1 },
    );
    cheat_caller_address_once(contract_address: reward_supplier, caller_address: staking_contract);
    let (strk_rewards, btc_rewards) = reward_supplier_dispatcher
        .update_current_epoch_block_rewards();
    // Update the expected avg_block_time.
    curr_avg_block_time =
        (weighted_avg_factor.into() * max_block_time
            + (100 - weighted_avg_factor.into()) * curr_avg_block_time)
        / 100;
    // Test avg_block_time.
    let avg_block_time = reward_supplier_dispatcher.get_avg_block_duration();
...
fn test_reward_supplier_eic() {
    ...
    assert!(reward_supplier_dispatcher.get_avg_block_duration() == DEFAULT_AVG_BLOCK_TIME);
    assert!(reward_supplier_dispatcher.get_block_time_config() == DEFAULT_BLOCK_TIME_CONFIG);

    // Upgrade.
    let avg_block_duration = DEFAULT_AVG_BLOCK_TIME - 10;
    let min_block_time = DEFAULT_BLOCK_TIME_CONFIG.min_block_time - 10;
    let max_block_time = DEFAULT_BLOCK_TIME_CONFIG.max_block_time - 10;
    let weighted_avg_factor = DEFAULT_BLOCK_TIME_CONFIG.weighted_avg_factor - 10;
    ...
    assert!(
        reward_supplier_dispatcher
            .get_block_time_config() == BlockTimeConfig {
                min_block_time, max_block_time, weighted_avg_factor,
            },
    );
}
...
#[test]
#[should_panic(expected: "EIC_LIB_CALL_FAILED")]
fn test_reward_supplier_eic_invalid_weighted_avg_factor() {
    let mut cfg: StakingInitConfig = Default::default();
    general_contract_system_deployment(ref :cfg);
    let reward_supplier = cfg.staking_contract_info.reward_supplier;
    let upgrade_governor = cfg.test_info.upgrade_governor;
...
#[test]
#[should_panic(expected: "EIC_LIB_CALL_FAILED")]
fn test_reward_supplier_eic_invalid_weighted_avg_factor_zero() {
    let mut cfg: StakingInitConfig = Default::default();
    general_contract_system_deployment(ref :cfg);
    let reward_supplier = cfg.staking_contract_info.reward_supplier;
    let upgrade_governor = cfg.test_info.upgrade_governor;
